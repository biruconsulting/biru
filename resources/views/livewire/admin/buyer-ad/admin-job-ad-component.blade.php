      ...
      <div class="card-header py-2">
        <div class="row">
          <div class="col-12 col-md-9 mt-2 d-flex justify-content-md-start justify-content-center">
            <p class="m-0 font-weight-bold text-primary">Buyer Job Ad DataTable</p>
          </div>
          ...
          <table class="table table-hover table-bordered text-center" width="100%" cellspacing="0">
            ...
            <tbody>
              @foreach ($buyer_job_ads as $buyer_job_ad)
                ...
                <tr>
                  <td>{{ $buyer_job_ad->id }}</td>
                  <td>{{ $buyer_job_ad->ad_title }}</td>
                  <td>{{ $buyer_job_ad->created_at }}</td>
                  <td>
                    ...
                    <a href="{{ route('buyer_ad.details', ['buyer_ad_id'=>$buyer_job_ad->id]) }}" class="btn btn-success btn-circle" title="View">
                      <i class="fas fa-eye"></i>
                    </a>
                    <a href="#" class="btn btn-danger btn-circle" wire:click.prevent="buyerJobAdDeleteConfirmation({{ $buyer_job_ad->id }})" title="Delete">
                        <i class="fas fa-trash"></i>
                    </a>
                  </td>
                </tr>
